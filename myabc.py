...
abc_bin = home_dir + '/Tools/abc/abc'
assert(os.path.isfile(abc_bin))
abclib = home_dir + '/Tools/abc/stdclibs/simpler.lib'
xorlib = home_dir + '/Tools/abc/stdclibs/xoronly.lib'
...
def resynthesize(in_file, out_file, synlib=abclib, quiet=False):
    ...
    cmd += "\nbalance; rewrite; rewrite -z; refactor; fraig; rewrite; balance;\n"
    cmd += "\nstrash"
    # cmd += "\ncollapse"
    # cmd += "\nsweep"
    cmd += "\nrewrite"
    ...
    if quiet:
        os.system("{0} < abc_script_gennet > /dev/null".format(abc_bin))
    else:
