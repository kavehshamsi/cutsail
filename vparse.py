...
class vlogTransformer(Transformer):
    ...
    def add_wires(self, st, cir):
        ...
        for wirename in wirenames:
            if nd.data == 'input_decl':
                ...
                if cir.find_wire(wirename) == None:
    ...
    def expand_var(self, cir, var):
        ...
        if hasattr(var, 'data'):
            if var.data == 'indexed_var':
                ...
                cvec = []
                #print('looking at constant ', dat_str)
                if '\'h' in dat_str:
                    bit_len, hex_str = dat_str.split('\'h')
                    bit_len = int(bit_len)
                    cvec = bin(int(hex_str, 16))[2:].zfill(bit_len)
                elif '\'b' in dat_str:
                    bit_len, bin_str = dat_str.split('\'b')
                    bit_len = int(bit_len)
                    cvec = bin(int(bin_str, 2))[2:].zfill(bit_len)
                elif '\'d' in dat_str:
                    bit_len, dec_str = dat_str.split('\'d')
                    bit_len = int(bit_len)
                    cvec = bin(int(dec_str, 10))[2:].zfill(bit_len)
                #print('cvec is ', cvec)
    ...
    def add_gate(self, st, cir):
        ...
        cir.add_gate(gate_fun, gate_ports[1:], gate_ports[0])
    ...
    def add_assignment(self, st, cir):
        ...
        for i in range(0, len(rhs_names)):
            cir.add_gate('BUF', [rhs_names[i]], lhs_names[i])
...
def read_verilog(infile):
    cirlist = parse_2_circuit_list(infile)
    assert len(cirlist) == 1
    return cirlist[0]
