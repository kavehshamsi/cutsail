...
class Cell:
    def __init__(self, cell_name, cell_port_names, port_dirs, fun=None):
    ...
    def __str__(self):
        ...
        for i in range(len(self.port_names)):
            retstr += ' ' + self.port_names[i] + ':' + str(self.port_dirs[i])
        retstr += ']  '
        if self.fun is not None:
            retstr += 'fun: ' + str(self.fun)
        return retstr
...
class CellLibrary:
    ...
    def add_cell_obj(self, cellobj):
        assert cellobj.cell_name not in self.cell_dict
        self.cell_dict[cellobj.cell_name] = cellobj

    def add_cell(self, cell_name, cell_ports, port_dirs, cellfun=None):
        port_inds = list()
        if cell_name not in self.cell_dict:
            cell = Cell(cell_name, cell_ports, port_dirs, cellfun)
...
class Circuit:
    ...
    def __init__(self, bench_file = None):
        ...
        self.num_inst = 0
        self.max_id = -1
        self.num_opened_wires = 0

        self.gp = nx.DiGraph()

        self.topsorted_gates = list()
        self.topsorted_gates_and_instances = list()
    ...
    def read_bench(self, bench_file):
        ...
        self.read_bench_from_fn(fin)
        fin.close()

    def read_bench_from_fn(self, fin):
        for ln in fin:
            ln = ln.replace(' ', '')
            ln = ln.strip()
            ...
            if 'INPUT(' in ln:
                ...
                if len(gate_strs) == 2:
                    self.add_gate(gfun.BUF, gate_strs[1], gate_strs[0])
                else:
                    self.add_gate(gate_strs[1], gate_strs[2:-1], gate_strs[0])
        return
    ...
    def add_gate_mappedwids(self, gate_fun, input_ids, output_id, conmap, gate_name=None):
        fanins = [conmap[xid] for xid in input_ids]
        fanout = conmap[output_id]
        return self.add_gate_wids(gatefun,  fanins, fanout, gate_name)

    # add gate using wire-ids
    def add_gate_wids(self, gate_fun, input_ids, output_id, gate_name=None):
        ...
        gate_name = 'g{0}'.format(self.num_gates) if gate_name is None else gate_name
        nx.set_node_attributes(self.gp, {gid: { ndattr.NODETYPE : ntype.GATE,
                                                ndattr.GFUNCTION : gateFunEnum,
                                                ndattr.NAME : gate_name } }
                               )
    ...
    def get_node_attr(self, nid, attr):
        return self.gp.nodes[nid][attr]

    # add gate using wire names
    def add_gate(self, gate_fun, input_names, output_name, gate_name=None):
    ...
    def add_instance(self, inst_name, cell_name, name_pairs):
        ...
        port_inds = []
        if cell_name not in self.cell_mgr.cell_dict:
            port_names = []
            port_dirs = []
            ...
            cell_obj = self.cell_mgr.cell_dict[cell_name]
            for i in range(len(name_pairs)):
                port_inds.append(cell_obj.port_names.index(name_pairs[i][0]))

        return self.add_instance_wids(inst_name, cell_name, port_wids, port_inds)


    def add_instance_wids(self, inst_name, cell_name, port_wids, port_inds):

        cellobj = self.cell_mgr.cell_dict[cell_name]
        ...
        for i in range(len(port_wids)):
            if cellobj.port_dirs[i] == portdir.OUT:
    ...
    def cell_of_instance(self, iid):
        return self.cell_mgr.cell_dict[self.gp.nodes[iid][ndattr.CELL_NAME]]

    def resolve_cell_directions(self, cell_out_names=['ZN', 'Z', 'Y']):
        for iid in self.instances():
            cell = self.cell_of_instance(iid)
            inst_ins = self.gp.nodes[iid][ndattr.PORT_WIDS]
            for i, port_ind in enumerate(self.gp.nodes[iid][ndattr.PORT_INDS]):
                if cell.port_names[port_ind] in cell_out_names:
                    print('setting port {} on instance {} as output '.format(port_ind, self.instance_str_vlog(iid)))
                    cell.port_dirs[i] == portdir.OUT
                    #try:
                    self.gp.remove_edge(inst_ins[i], iid)
                    self.gp.add_edge(iid, inst_ins[i])
    ...
    def write_verilog(self, out=sys.stdout):
        print(self.to_verilog(), file=out)
    ...
    def instance_str_bench(self, instid):
        inst = self.gp.nodes[instid]
        #print(inst)
        retstr = str()
        conn_pairs = list()
        cell = self.cell_mgr.cell_dict[inst[ndattr.CELL_NAME]]
        for i, port_ind in enumerate(inst[ndattr.PORT_INDS]):
            if cell.port_dirs[i] == portdir.OUT:
                conn_pairs.append('.{0}({1})'.format(cell.port_names[port_ind], self.name(inst[ndattr.PORT_WIDS][i])))
        retstr += list_strs(conn_pairs, ', ') + ' = '
        conn_pairs.clear()
        retstr += '{0}('.format(inst[ndattr.CELL_NAME])
        ...
        # for fo in self.fanouts(instid):
        #     print('fo: ', self.name(fo))
        # for fo in self.fanins(instid):
        #     print('fi: ', self.name(fo))
        return retstr
    ...
    def to_bench(self):
        ...
        retstr += f'# {self.circuit_name}\n'
        retstr += f'# {self.header}\n'
        ...
        for wid in self.out_ids:
            retstr += 'OUTPUT({0})\n'.format(self.name(wid))
        for iid in self.instances():
            retstr += '{0}\n'.format(self.instance_str_bench(iid))
        for gid in self.gates():
            retstr += self.gate_str(gid)
        return retstr
    ...
    def gate_str(self, nid):
        ...
        gout = self.fanout(nid)
        assert gout is not None
        fanout_name = self.name(gout)
    ...
    def insert_gate(self, gatefun, wid, wname='opened'):
        ...
        return self.add_gate_wids(gatefun, inputs, right_wid)
    ...
    def set_wiretype(self, wid, wtype):
        ...
        if old_wtype != wtype:
            ...
            self.gp.nodes[wid][ndattr.NODETYPE] = wtype
    ...
    def add_circuit(self, cir, connMap, postfix=None):
        ...
        for gid in cir.gates():
            ...
            self.add_gate_wids(cir.gatefun(gid), newfanins, newfanout)
    ...
    def join_outputs(self, fun, oname=None, joinset=None):
        ...
        if len(joinset) == 1:
            if fun == gfun.NOT or fun == gfun.NAND or fun == gfun.XNOR or fun == gfun.NOR:
                self.add_gate_wids(gfun.NOT, list(joinset), noid)
            else:
                self.add_gate_wids(gfun.BUF, list(joinset), noid)
        else:
            self.add_gate_wids(fun, list(joinset), noid)
    ...
    def gates_and_instances(self):
        for nid in self.gp.nodes():
            if self.is_gate(nid) or self.is_inst(nid):
                yield nid
    ...
    def fanoutfanouts(self, nid):
        retset = set()
        for nid1 in self.fanouts(nid):
            for nid2 in self.fanouts(nid1):
                retset.add(nid2)
        return retset

    def faninfanins(self, nid):
        retset = set()
        for nid1 in self.fanins(nid):
            for nid2 in self.fanins(nid1):
                retset.add(nid2)
        return retset
    ...
    def neighbors(self, gid):
        retset = set(self.fanins(gid))
        retset.update(self.fanouts(gid))
        return retset

    def neighborgs(self, gid):
        assert self.is_gate(gid)
        retset = set(self.gfanings(gid))
    ...
    def numfanins(self, id):
        return len(list(self.fanins(id)))

    def numfanouts(self, id):
        return len(list(self.fanouts(id)))
    ...
    def node_exists(self, nid):
        return nid in self.gp.nodes

    def is_wire(self, wid):
        return self.node_exists(wid) and self.nodetype(wid) != ntype.GATE
    ...
    def topsort(self):
        ...
        for nd in topsorted_nodes:
            if self.is_gate(nd):
                ...
                self.topsorted_gates_and_instances.append(nd)
            elif self.is_inst(nd):
                self.topsorted_gates_and_instances.append(nd)
            elif self.is_wire(nd):
    ...
    def simulate(self, sim_map):
        ...
        for gid in self.topsorted_gates_and_instances:

            if self.is_inst(gid):
                cell_name = self.get_node_attr(gid, ndattr.CELL_NAME)
                port_wids = self.get_node_attr(gid, ndattr.PORT_WIDS)
                port_inds = self.get_node_attr(gid, ndattr.PORT_INDS)
                cell_obj = self.cell_mgr.cell_dict[cell_name]
                if cell_obj.fun is None:
                    raise Exception('no simulation model available for instance {} of cell {}'.format(self.name(gid), cell_name))
                if isinstance(cell_obj.fun, Circuit):
                    funcir = cell_obj.fun
                    cell_smap = dict()
                    for i in range(len(port_wids)):
                        pwid = port_wids[i]
                        pind = port_inds[i]
                        if cell_obj.port_dirs[pind] == portdir.IN:
                            cell_smap[funcir.find_wcheck(cell_obj.port_names[pind])] = sim_map[pwid]
                    funcir.simulate(cell_smap)
                    for i in range(len(port_wids)):
                        pwid = port_wids[i]
                        pind = port_inds[i]
                        if cell_obj.port_dirs[pind] == portdir.OUT:
                            sim_map[pwid] = cell_smap[funcir.find_wcheck(cell_obj.port_names[pind])]
                continue
            ...
            if gf == gfun.BUF:
                ...
                raise Exception('unsupported type', gf, 'for simulation')
    ...
    def link_to_library(self, cell_lib):
        for inst in self.instances():
            cell_name = self.gp.nodes[inst][ndattr.CELL_NAME]
            port_wids = self.gp.nodes[inst][ndattr.PORT_WIDS]
            port_inds = self.gp.nodes[inst][ndattr.PORT_INDS]
            cell_obj = self.cell_mgr.cell_dict[cell_name]

            if cell_name not in cell_lib.cell_dict:
                print('cannot link instance ', self.name(inst), ' of cell type ', cell_name)
                continue

            link_cell = cell_lib.cell_dict[cell_name]
            #print('linking instance ', self.name(inst), ' of cell type ', cell_name, ' to cell ', link_cell)

            for i in range(len(port_wids)):
                pind = port_inds[i]
                pwid = port_wids[i]
                lpind = port_inds[i] = link_cell.port_names.index(cell_obj.port_names[pind])
                if cell_obj.port_dirs[pind] != link_cell.port_dirs[lpind]:
                    #print('need to change the direction of {} from {} to {} for wire {}'.format(link_cell.port_names[lpind], str(cell_obj.port_dirs[pind]), str(link_cell.port_dirs[lpind]), self.name(pwid)))
                    #print(self.instance_str_bench(inst))
                    if link_cell.port_dirs[lpind] == portdir.IN and cell_obj.port_dirs[pind] != portdir.INOUT:
                        self.gp.remove_edge(inst, pwid)
                        self.gp.add_edge(pwid, inst)
                    elif link_cell.port_dirs[lpind] == portdir.OUT:
                        self.gp.remove_edge(pwid, inst)
                        self.gp.add_edge(inst, pwid)

        self.cell_mgr = cell_lib

    def instances_to_gates(self):
        for inst in self.instances():
            self.instance_str_vlog(inst)
        print('all instances are single ended')
        gid2cell = dict()
        for inst in copy.deepcopy(self.instances()):
            cell_name = self.gp.nodes[inst][ndattr.CELL_NAME]
            port_wids = self.gp.nodes[inst][ndattr.PORT_WIDS]
            port_inds = self.gp.nodes[inst][ndattr.PORT_INDS]
            cell_obj = self.cell_mgr.cell_dict[cell_name]
            ...
            for i in range(len(port_wids)):
                pind = port_inds[i]
                pwid = port_wids[i]
                pdir = cell_obj.port_dirs[i]
                print(pdir)
                if pdir == portdir.OUT:
                    gate_out = pwid
                elif pdir == portdir.IN:
                    gate_ins.append(pwid)
                else:
                    raise Exception('cannot convert instance ' + self.name(inst) + ' of type ' + cell_name + ' with INOUT port to gate')
            gid = self.add_gate_wids(gfun.INST, gate_ins, gate_out)
            gid2cell[gid] = cell_name
            self.remove_instance(inst)

        return gid2cell

    def toDot(self, colorfun=None):
        id2ndname = dict()
        graph = graphviz.Digraph()

        if colorfun is None:
            colorfun = lambda xid: 'green' if self.is_input(xid) else 'red' if self.is_key(xid) else 'blue' if self.is_output(xid) else 'gray'

        for xid in self.allports():
            nm = id2ndname[xid] = self.name(xid)
            graph.node(nm, label=nm, shape='egg', style='filled', fillcolor=colorfun(xid))
        for gid in self.gates():
            nm = id2ndname[gid] = self.name(gid)
            graph.node(nm, label=self.gatefun(gid).name.lower(), shape='triangle', orientation='180', style='filled', fillcolor=colorfun(gid))
        for iid in self.instances():
            nm = id2ndname[iid] = self.name(iid)
            graph.node(nm, label=self.cell_of_instance(iid).cell_name, shape='triangle', orientation='180', style='filled', fillcolor=colorfun(iid))

        for xid in self.allins():
            for gid in self.fanouts(xid):
                graph.edge(self.name(xid), self.name(gid))
        for gid in self.gates():
            wout = self.fanout(gid)
            if len(list(self.fanouts(wout))) == 0:
                graph.edge(self.name(gid), self.name(wout))
            else:
                for gid2 in self.fanouts(wout):
                    graph.edge(self.name(gid), self.name(gid2))

        for iid in self.instances():
            wout = self.fanout(iid)
            if self.is_output(wout):
                graph.edge(self.name(iid), self.name(wout))
            else:
                for gid2 in self.fanouts(wout):
                    graph.edge(self.name(iid), self.name(gid2))

        return graph, id2ndname
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 3:
        print('usage: circuit.py write_bench <input_bench>')
        print('usage: circuit.py draw <input_bench>')
        exit(1)

    if sys.argv[1] == 'write_bench':
        cir = Circuit(sys.argv[2])
        cir.write_bench()
    elif sys.argv[1] == 'draw':
        cir = Circuit(sys.argv[2])
        graph, _ = cir.toDot()
        graph.render('./dotfile', view=True)
